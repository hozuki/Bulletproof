...
// Test case source code grabbed from http://jabbany.github.io/CommentCoreLibrary/demo/scripting/
// Modified by Hozuki, Aug. 2015 for testing
...
if (filters)
    ball.filters = [$.createGlowFilter(0xff0000, 1, 40, 40, 20)];
var mrgb = 0xff0000;
...
function render() {
    ...
    if (frameCount === 0 && filters) {
        ...
        if (nmrgb !== mrgb) {
            mrgb = nmrgb;
            ball.filters = [$.createGlowFilter(mrgb, 1, 40, 40, 20)];
        }
    }

    if (wireframe)
        ball.graphics.lineStyle(1, fill ? 0xffffff : color, 1);
    if (fill)
        ball.graphics.beginFill(0xffffff);
    ball.graphics.drawTriangles(projectedVertices, indices, null, 'negative');
    if (fill)
        ball.graphics.beginFill(color);
    ball.graphics.drawTriangles(projectedVertices, indices, null, 'positive');
    ball.graphics.endFill();
}

init();
ball.addEventListener('enterFrame', render);
$G._set('unload', function () {
    ball.removeEventListener('enterFrame', render);
