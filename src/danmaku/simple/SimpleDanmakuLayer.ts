...
export class SimpleDanmakuLayer extends TextField {
    ...
    protected __drawSimpleDanmaku(context2D:CanvasRenderingContext2D, danmaku:SimpleDanmaku, last:SimpleDanmaku):void {
        ...
        if (_util.isUndefinedOrNull(last) || cp.fontName !== last.createParams.fontName || cp.fontSize !== last.createParams.fontSize) {
        ...
        if (cp.background) {
            context2D.fillStyle = _util.colorToCssSharp(cp.backgroundColor);
            context2D.fillRect(x, y, textWidth + borderThickness * 2, textHeight + borderThickness * 2);
        }
        context2D.fillStyle = _util.colorToCssSharp(cp.textColor);
        ...
        if (cp.outline && cp.outlineThickness > 0) {
            ...
            context2D.strokeStyle = _util.colorToCssSharp(cp.outlineColor);
        ...
        if (cp.border && cp.borderThickness > 0) {
            ...
            context2D.strokeStyle = _util.colorToCssSharp(cp.borderColor);
